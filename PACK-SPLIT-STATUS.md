...
| `krea2-txt2img-manual` | krea2 | text→image (prose) | ✅ | **NEW** — Krea 2 Turbo (Krea.ai 12B DiT, distilled), prose prompt. Native krea2 + Qwen3-VL 4B encoder + Qwen image VAE, 8 steps cfg 1. Standalone, no group toggles (manual prompt active, JSON-builder branch removed). Smart Seed Variance + ConditioningKrea2Rebalance left BYPASSED (optional opt-ins). ImageSharpenKJ (rcas 0.55) active (converter gap: bypass-passthrough doesn't cross a subgraph IMAGE output). Render-verified (crisp snow-leopard) |
| `krea2-txt2img-json` | krea2 | text→image (JSON builder) | ✅ | **NEW** — same krea2 turbo stack driven by Ideogram4PromptBuilderKJ (per-region desc + bboxes). Standalone, no group toggles (JSON builder active, manual branch removed; seed-variance + rebalance bypassed opt-ins). Render-verified (tea still-life, each object in its bbox). JSON prompting needs ALL builder fields set + EMPTY palettes (else celebrity bleed / swatch-strip) — shipped default is a clean still-life. Split from the old combined `krea2-txt2img` (deleted along with the `krea2` monolith) |
