...
[official MCP server](https://mcp.civitai.com/mcp). Add both to your session
and Claude discovers models on Civitai, then installs and wires them locally
through comfyui-mcp:
...
the graph. Browsing Civitai works without a key; the key unlocks your account
context and gated downloads. The `civitai` skill teaches Claude the full
discovery → download → generate handoff (and falls back to HuggingFace search
when the Civitai MCP isn't connected).
