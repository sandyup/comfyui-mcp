...
Arena results (RTX 4090, ComfyUI 0.27, temperature 0):

| Model | health | models | registry | queue | generate | Score |
| --- | --- | --- | --- | --- | --- | --- |
| `qwen3:4b` (2.6 GB) | ✅ | ✅ | ✅ | ✅ | 🟡 | **9/10** |
| `gemma4:e4b` (9.6 GB) | ✅ | ✅ | ✅ | ✅ | 🟡 | **9/10** |
| `gemma4:e2b` (7.2 GB) | ✅ | ✅ | ✅ | ✅ | ❌ | **8/10** |
| `qwen3:8b` (5.2 GB) | ✅ | ✅ | ✅ | ❌ | 🟡 | **7/10** |
| `llama3.1:8b` (4.9 GB) | ❌ | ❌ | ✅ | ❌ | ❌ | **2/10** |
| `gemma3` (any size) | — | — | — | — | — | ❌ no native tool calling |

✅ completed & server-verified · 🟡 right tool family, incomplete outcome · ❌ failed

Takeaways: single-tool tasks (health, installed models, registry search,
queue) are solidly in reach for the qwen3/gemma4 class. The open frontier is
the **async multi-hop task** — start a generation, poll its job status,
report the output — where every capable model built a workflow but stopped
short of enqueue-and-poll. llama3.1:8b's tool-format discipline collapses on
this catalog (it hallucinates tool names and prints tool-call JSON as text).
Gemma 4 shipped native function calling across the family (Ollama ≥ v0.20);
`e4b` or larger is the sweet spot.
