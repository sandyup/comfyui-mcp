---
title: "How it works"
description: "The model behind the tools — transports, the ComfyUI-Manager API, and local vs. remote."
...
<Steps>
  ...
  <Step title="Install & filesystem ops → local only">
    ...
    `COMFYUI_PATH` and return a clear error in remote mode.
  </Step>
</Steps>

<Note>
  Rule of thumb: anything that **reads or runs** the connected server works locally and
  remotely; anything that **installs software or touches files on disk** needs a local install.
</Note>
