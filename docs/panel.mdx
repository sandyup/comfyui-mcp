---
title: "Sidebar Panel"
description: "ComfyUI Agent Panel — an autonomous AI agent in ComfyUI's sidebar that drives your canvas, now on EITHER Claude or ChatGPT (your own subscription, no API key). Available on the Comfy Registry as comfyui-agent-panel."
icon: "comments"
---
...
**Claude** or **ChatGPT** — and the matching agent runs **in the background on
your own subscription**, at [full feature parity](/backends).

```
you ⇄ panel (pick a provider) ⇄ loopback bridge ⇄ panel orchestrator ⇄ background agent: Claude OR ChatGPT (your subscription)
```

**No API keys, no per-token billing.** The agent authenticates with your on-disk
Claude *or* ChatGPT login. Each provider runs its own orchestrator on its own
loopback bridge port (Claude defaults to `ws://127.0.0.1:9180`), so you pick a
provider rather than a port. The bridge is loopback-only, and the panel executes a
**fixed allowlist** of graph commands (no arbitrary JavaScript).
...
3. **Restart ComfyUI**, open the **Agent** tab, **pick a provider** (Claude /
   ...
   orchestrator (`npx -y comfyui-mcp --panel-orchestrator`) on your subscription and
